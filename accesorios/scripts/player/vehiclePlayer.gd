...
func _process(delta):
	...
	var acceleration = eng_rotation_speed*tr_rotation_speed
	var rpm = $rr.get_rpm()
	$rr.engine_force = (acceleration * dif)*clutch
	rpm = $rl.get_rpm()
	$rl.engine_force = (acceleration * dif)*clutch
	#gearing
	if rpm < 100 && current_gear == 2:
		print("engine stalled!")
		eng_rotation_speed = 0
		clutch = 0
	#
	elif rpm < 200 && current_gear == 3:
		print("engine stalled!")
		eng_rotation_speed = 0
		clutch = 0
	elif rpm < 300 && current_gear == 4:
		eng_rotation_speed = 0
		clutch = 0
	elif rpm < 400 && current_gear == 5:
		eng_rotation_speed = 0
		clutch = 0
	#changing camera
